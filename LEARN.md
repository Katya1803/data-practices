...
### 3.6 Spring Boot Consumer — Realtime Stats

Project có **hai consumer** nhận cùng 3 topic, nhưng hoạt động hoàn toàn độc lập:

| | Django Consumer | Spring Boot Consumer |
|--|----------------|---------------------|
| **Group ID** | `dvdrental-group` | `springboot-stats-group` |
| **Làm gì** | Ghi event vào PostgreSQL (activity_log) | Tăng counter trong Redis |
| **Kết quả** | Trang Activity hiển thị lịch sử | `GET /api/stats/realtime` trả thống kê ngày |

Vì khác consumer group → Kafka giao **bản sao riêng** của mỗi message cho từng group. Cả hai đều nhận đủ 100% event.

**Tại sao dùng `StringDeserializer` thay vì `JsonDeserializer`?**

Producer cấu hình `spring.json.add.type.headers=false` — nghĩa là message JSON không kèm header kiểu dữ liệu. `JsonDeserializer` của Spring cần header đó để biết deserialize thành class nào. Dùng `StringDeserializer` nhận raw JSON string rồi tự parse bằng `ObjectMapper` là cách đơn giản và an toàn hơn khi nhiều topic có kiểu khác nhau.

**Consumer hoạt động** — `springboot/.../kafka/consumer/RealtimeStatsConsumer.java`:
```java
@KafkaListener(
    topics = "rental.created",
    containerFactory = "statsListenerContainerFactory"   // factory dùng StringDeserializer
)
public void onRentalCreated(String payload) {
    JsonNode node = objectMapper.readTree(payload);      // parse JSON thủ công
    statsService.incrementRentals(LocalDate.now());      // INCR trong Redis
}
```

**Redis lưu counter theo ngày:**
```
stats:rentals:2026-04-22   = 5    ← INCR mỗi lần rental.created đến
stats:returns:2026-04-22   = 3    ← INCR mỗi lần rental.returned đến
stats:payments:2026-04-22  = 4    ← INCR mỗi lần payment.processed đến
stats:revenue:2026-04-22   = 17.96 ← INCRBYFLOAT cộng dồn amount
```

**Thử thực hành:**
1. Tạo một rental trên trang `http://localhost:5173/rentals`
2. Chạy ngay: `curl http://localhost:8080/api/stats/realtime` → thấy `rentalsCreated` tăng
3. Mở ARDM → tìm key `stats:*` → thấy các counter theo ngày hôm nay
4. Trong Offset Explorer → **Consumers** → `springboot-stats-group` → lag = 0 (đã xử lý hết)
